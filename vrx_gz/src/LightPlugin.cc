...
class LightBuoyPlugin::Implementation
{
  ...
  public: std::uniform_int_distribution<int> dist_int{0, 3};
};
...
bool LightBuoyPlugin::Implementation::ParseSDF(sdf::ElementPtr _sdf)
{
  ...
  int dist_int_value = dist_int(gen);
  std::cout << "Random int value: " << dist_int_value << std::endl;
  if (dist_int_value == 0 || dist_int_value == 2) {
    colors = {"color_1", "color_1", "color_1"};
  } else if (dist_int_value == 1 || dist_int_value == 3) {
    colors = {"color_2", "color_2", "color_2"};
  }

  for (auto colorIndex : colors)
  {
  ...
  while (visualElem)
  {
    ...
    std::cout << "Visual name - " << visualName << std::endl;
...
void LightBuoyPlugin::Implementation::Update()
{
  ...
  this->nextUpdateTime += std::chrono::duration<double>(86400.0);
